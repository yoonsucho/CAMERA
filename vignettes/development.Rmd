...
This returns a set of LD clumped SNPs that are GWAS significant for BMI (raw instruments). The following function identifies SNPs that have strong associations with the exposure in each population. This is the same method as instrument extraction for multivariable MR. A data frame of the extracted instruments is stored in \code{x$instrument_raw}
...
Also, instruments can be obtained using fine mapping method. Extracted instruments are stored in \code{x$instrument_susie} or \code{x$instrument_susie}
```r
